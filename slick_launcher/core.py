...
class CoreLogic:
    ...
    """
    def __init__(self):
        self.config_dir = user_config_dir("slick_launcher", "slick_launcher_author") # Added appauthor for robustness
    ...
    def load_plugins(self, api_instance: API, app_settings: Settings): # Changed signature
        """
        ...
        for plugin_class in plugins_registry:
            try:
                ...
                if hasattr(plugin_instance, 'register_settings') and callable(plugin_instance.register_settings):
        ...
        loaded_plugins_temp.sort(key=lambda p: (
            not (p.PREFIX or p.SUFFIX), 
            p.PRIORITY,                 
            not p.IS_DEFAULT            
        ))
        ...
        if not self.default_plugin and self.plugins:
            self.default_plugin = self.plugins[-1] # Fallback: last by priority if no explicit default
            if self.default_plugin:
                 print(f"Core: No explicit default plugin. Using '{self.default_plugin.NAME}' as fallback default.", file=sys.stderr)
    ...
    def reset_history_index_to_latest(self):
        if not self.ignore_text_changed_for_history: # Only reset if not actively navigating
            self.current_history_index = len(self.history)

    def find_plugin(self, command: str = "", is_default: bool = False) -> Optional[PluginInterface]:
        if is_default:
    ...
    def cleanup_plugins(self):
        for plugin in self.plugins:
            try:
                ...
                print(f"Core: Error cleaning up plugin {getattr(plugin, 'NAME', 'UnknownPlugin')}: {e}", file=sys.stderr)
                traceback.print_exc()
