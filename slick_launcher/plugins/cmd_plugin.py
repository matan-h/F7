    ...
    def run(self):
        try:
            ...
            if proc.returncode != 0:
                ...
                if stderr and stderr.strip(): # Include stderr even on success if it has content
                    output += f"\n[stderr]\n{stderr.strip()}"
                self.finished.emit(output.strip())
        except FileNotFoundError:
            self.error.emit(f"Execution error: Command or shell not found for '{self.cmd.split()[0]}'. Ensure it's in your PATH.")
...
class CmdPlugin(PluginInterface):
    ...
    def __init__(self, api_instance, settings): # Corrected type hint
        ...
        self.current_cmd_for_preview = "" # To avoid re-running identical preview commands
    ...
    def update_preview(self, command: str, selected_text: str, manual: bool) -> None:
        ...
        else: # Should not happen if plugin is correctly matched by prefix
            self.api.hide_preview()
            return
        ...
        if cmd_part == self.current_cmd_for_preview and not manual and self.worker and self.worker.isRunning():
        ...
        self.worker.error.connect(
            lambda err_msg: self.api.set_status(f"❌ Preview error: {err_msg[:30]}...", self.NAME)
        )
        self.worker.start()
    ...
    def execute(self, command: str, selected_text: str) -> Optional[str]:
        ...
        try:
            ...
            stdout, stderr = proc.communicate(input=input_to_send, timeout=15) # Added timeout

            if proc.returncode != 0:
                error_message = f"CMD Error (code {proc.returncode})"
                if stderr and stderr.strip(): error_message += f": {stderr.strip()}"
                elif stdout and stdout.strip(): error_message += f": {stdout.strip()}" # Error might be on stdout
            ...
            self.api.set_status(f"Timeout executing: {actual_command_to_run}", self.NAME)
            self.api.update_preview_content(f"Error: Command '{actual_command_to_run}' timed out after 15 seconds.")
