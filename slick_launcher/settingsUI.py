...
from PyQt6.QtWidgets import (
    QDialog, QDoubleSpinBox, QTabWidget, QWidget, QFormLayout, QLabel,
    QCheckBox, QLineEdit, QComboBox, QSpinBox, QScrollArea,
    QDialogButtonBox, QMessageBox, QVBoxLayout, QPushButton, QColorDialog, QApplication, QTextEdit,
    QHBoxLayout, QKeySequenceEdit # Added QKeySequenceEdit
)
import sys

from PyQt6.QtCore import Qt, pyqtSignal
from PyQt6.QtGui import QColor, QScreen, QValidator, QKeySequence # Added QKeySequence

from .settings import Settings, Color, HotKeyType
...
class SettingsDialog(QDialog):
    ...
    def __init__(self, settings: Settings, parent=None):
        ...
        for section in settings._registry:
            ...
            for name, meta in settings._registry[section].items():
                ...
                desc_label.setStyleSheet("color: #7f8c8d; font-size: 12px; margin-left: 5px; margin-top: -5px;")
                desc_label.setWordWrap(True)

                current_value = self.settings._values.get(section, {}).get(name, meta.get('default'))
                ...
                if setting_type == bool:
                    ...
                    value_widget.setChecked(current_value if isinstance(current_value, bool) else False)
                    widget_to_add_to_layout = value_widget
                elif setting_type == str and meta.get('options'):
                    value_widget = QComboBox()
                    value_widget.addItems(meta['options'])
                    initial_text = current_value if isinstance(current_value, str) and current_value in meta['options'] else (meta['options'][0] if meta['options'] else "")
                    ...
                    if isinstance(default_text, str) and '\n' in default_text:
                        ...
                        value_widget.setPlainText(current_value if isinstance(current_value, str) else "")
                    else:
                        value_widget = QLineEdit()
                        value_widget.setText(current_value if isinstance(current_value, str) else "")
                    widget_to_add_to_layout = value_widget
                elif setting_type == int:
                    value_widget = QSpinBox()
                    value_widget.setMinimum(meta.get('min', -1000))
                    value_widget.setMaximum(meta.get('max', 1000))
                    value_widget.setValue(current_value if isinstance(current_value, int) else 0)
                    widget_to_add_to_layout = value_widget
                    ...
                    value_widget.setValue(float(current_value) if isinstance(current_value, (int, float)) else 0.0)
                    widget_to_add_to_layout = value_widget
                elif setting_type == Color:
                    color_button = QPushButton()
                    try:
                        q_color = QColor(current_value if isinstance(current_value, str) else "#ffffffff")
                    except Exception:
                        q_color = QColor("#ffffffff")
                    if not q_color.isValid(): q_color = QColor("#ffffffff")

                    hex_argb = color2h(q_color)
                    color_button.setStyleSheet(f"background-color: {hex_argb}; border-radius: 4px; padding: 8px; min-width: 30px;")
                    ...
                    clear_btn.clicked.connect(key_sequence_edit.clear) # Use QKeySequenceEdit's clear
                    clear_btn.setFixedWidth(clear_btn.fontMetrics().horizontalAdvance(" Clear ") + 10)
                    ...
                    print(f"WARN: unknown type in settings: {section}.{name} ({meta['type']})", file=sys.stderr)
                    widget_to_add_to_layout = QLabel(f"Unsupported type: {meta['type']}")
                ...
                if enabled_widget and value_widget:
                    ...
                    if setting_type == HotKeyType and widget_to_add_to_layout:
                        clear_button_in_container = widget_to_add_to_layout.findChild(QPushButton)
                        if clear_button_in_container:
                             clear_button_in_container.setEnabled(enabled_widget.isChecked())
                             enabled_widget.toggled.connect(clear_button_in_container.setEnabled)
    ...
    def pick_color(self, button: QPushButton):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            button.setStyleSheet(f"background-color: {hex_argb}; border-radius: 4px; padding: 8px; min-width: 30px;")
    ...
    def apply_changes_to_settings(self):
        ...
        try:
            for (section, name), widget_data in self.widget_map.items():
                ...
                value_widget = widget_data['value'] # This is QKeySequenceEdit for HotKeyType
                ...
                if enabled_widget and not enabled_widget.isChecked():
                    ...
                    if setting_type == bool:
                        ...
                        qks = value_widget.keySequence() # value_widget is QKeySequenceEdit
                        ...
                        if text_val.strip():
                            new_value = [item.strip() for item in text_val.split(',') if item.strip()]
                        else:
                            new_value = []
                ...
                if new_value is None and is_nullable:
                    ...
                    if setting_type == Color: is_correct_type = isinstance(new_value, Color)
                    elif setting_type == HotKeyType: is_correct_type = isinstance(new_value, HotKeyType)
                    elif setting_type == list: is_correct_type = isinstance(new_value, list)
                    else: is_correct_type = isinstance(new_value, setting_type)

                    if is_correct_type:
                        self.settings._values[section][name] = new_value
                    else:
                        print(f"Warning: Type mismatch for {section}.{name}. Expected {setting_type}, got {type(new_value)} with value '{new_value}'. Not updating.", file=sys.stderr)
                elif not is_nullable and new_value is None:
                    if setting_type == str: self.settings._values[section][name] = ""
                    elif setting_type == HotKeyType: self.settings._values[section][name] = HotKeyType("")
                    elif setting_type == Color: self.settings._values[section][name] = Color("")
                    elif setting_type == list: self.settings._values[section][name] = []
                    print(f"Info: Non-nullable field {section}.{name} got None. Setting to empty/default if possible.", file=sys.stderr)
