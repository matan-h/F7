...
if __name__ == "__main__":
            ...
            self._registry = {
                "General": {
                    ...
                    "launch_hotkey": {
                        "type": HotKeyType,
                        "default": "<ctrl>+<alt>+s",
                        "description": "Global hotkey to launch the app.",
                    },  # Custom format
                    "nullable_hotkey": {
                        "type": HotKeyType,
                        "default": None,
                        "description": "Optional hotkey, can be empty.",
                    },
                    "f_key_hotkey": {
                        "type": HotKeyType,
                        "default": "<shift>+<f11>",
                        "description": "Shortcut with an F-key.",
                    },
                    "just_key_hotkey": {
                        "type": HotKeyType,
                        "default": "<space>",
                        "description": "Shortcut with just space key.",
                    },
                },
                "Appearance": {
                    "theme_color": {
                    ...
                    "ignored_files": {
                        "type": list,
                        "default": [".git", "*.tmp", "__pycache__"],
                        "description": "Comma-separated list of files/patterns to ignore.",
                    },
                    "nullable_list": {
                        "type": list,
                        "default": None,
                ...
                "Advanced": {
                    ...
                    "multiline_text": {
                        "type": str,
                        "default": "Hello\nWorld\nThis is a test.",
                        "description": "A multiline text setting.",
                    },
                },
            }
            self._values = {
    ...
    for cs in test_custom_strs:
        ...
        if not cs and not cs_back:
            match = True  # "" -> QKeySequence() -> None, consider match for empty

        print(
            f"Custom: '{cs}' -> QKS: '{qks.toString(QKeySequence.SequenceFormat.PortableText)}' -> Custom Back: '{cs_back}' (Match: {match})"
        )
    ...
    for qks_str in test_qks_strs:
        ...
        match = qks == qks_back
        print(
            f"QKS: '{qks_str}' -> Custom: '{cs}' -> QKS Back: '{qks_back.toString(QKeySequence.SequenceFormat.PortableText)}' (Match: {match})"
        )
    print("----------------------------------\n")
    ...
    dialog.settingsApplied.connect(
        lambda: print(
            f"SettingsApplied signal received! Current settings:\n{mock_settings_instance}"
        )
    )
