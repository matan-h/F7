...
try:
    ...
    print("Warning: pyperclip library not found. Clipboard operations (including fallback method) will fail.", file=sys.stderr)
...
try:
    ...
    print("Warning: pyautogui library not found. Clipboard simulation fallback method will fail.", file=sys.stderr)
        ...
        if wl_paste_path:
            try:
                ...
                if result.stdout:
                 ...
                 print("wl-paste command not found despite initial check.", file=sys.stderr)
                ...
                pass
                 ...
                 print(f"An unexpected error occurred with wl-paste: {e}", file=sys.stderr)
        # else:
...
def _get_selected_text_clipboard_hack():
    ...
    if not pyperclip or not pyautogui:
        print("Error: Clipboard hack requires 'pyperclip' and 'pyautogui'.", file=sys.stderr)
        return None
    ...
    try:
        ...
        try:
            selected_text = pyperclip.paste()
             ...
             print(f"Error: Could not read clipboard after copy simulation: {e}", file=sys.stderr)
        ...
        if original_clipboard is not None:
            ...
            if current_clipboard_after_read != original_clipboard:
                 try:
                    ...
                    print(f"Error: Failed to restore original clipboard content: {restore_e}", file=sys.stderr)
            # else:
...
def get_selected_text(allow_clipboard_hack=True):
    ...
    # 3. If direct method failed and hack wasn't allowed or also failed
         ...
         print("Direct method failed or not applicable, and clipboard hack is disallowed.", file=sys.stderr)
